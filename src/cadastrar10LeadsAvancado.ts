import { criarLead } from "./utils/criarLead";
 ...
 * Preço: R$7, R$15, R$30
 */
function calcularScore(tipo: string, valor: number, urgencia: string) {
  let score = 0;

  // Urgência
  switch (urgencia) {
    case "economico": score += 10; break;
    case "qualificado": score += 25; break;
    case "prioritario": score += 50; break;
    default: score += 10;
  }

  // Tipo
  switch (tipo) {
    case "compra": score += 30; break;
    case "aluguel": score += 20; break;
    case "venda": score += 15; break;
    case "anunciar": score += 10; break;
  }

  // Valor
  if (valor <= 264000) score += 5;
  else if (valor <= 1000000) score += 10;
  else score += 15;
